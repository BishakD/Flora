      ...
      <AdminShell title="Room Types">
        ...
        <div className="mb-6 flex items-center justify-between gap-4">
          <p className="font-sans text-[0.78rem] text-flora-grey">
            {loading ? "Loading…" : `${rooms.length} room type${rooms.length !== 1 ? "s" : ""}`}
          </p>
          <Link
            ...
            <table className="w-full border-collapse font-sans text-[0.8rem] text-flora-charcoal">
              <thead>
                <tr className="border-b border-flora-line bg-flora-cream/60 text-left">
                  {["Thumbnail", "Name", "Eyebrow", "Size", "Max guests", "Amenities", "Actions"].map(
                    (col) => (
                      <th
                        key={col}
              ...
              <tbody>
                {rooms.map((room, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-flora-grey">{room.eyebrow}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-flora-grey">{room.size_label}</td>
                    <td className="px-4 py-3 text-center">{room.max_guests}</td>
                    <td className="px-4 py-3 text-center">{room.amenities?.length ?? 0}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link
                          href={`/admin/room-types/${room.id}/edit`}
                          className="luxury-button border-flora-blue text-flora-slate [--button-fill:var(--flora-blue)] [--button-ink:var(--flora-ivory-card)] px-3 py-1 text-[0.56rem]"
                        >
                          Edit
                        </Link>
                        <button
                          ...
                          className="luxury-button border-flora-rose text-flora-terracotta [--button-fill:var(--flora-blush)] [--button-ink:var(--flora-espresso)] px-3 py-1 text-[0.56rem]"
                        >
                          Delete
                        </button>
                      </div>
